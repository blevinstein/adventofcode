...
async function main() {
  ...
  function distance(a, b) {
    return Math.abs(a[0] - b[0])
        + Math.abs(a[1] - b[1])
        + emptyRows.filter(i => i > Math.min(a[0], b[0]) && i < Math.max(a[0], b[0])).length * (multiplier - 1)
        + emptyCols.filter(j => j > Math.min(a[1], b[1]) && j < Math.max(a[1], b[1])).length * (multiplier - 1);
  }
