...
async function main() {
  ...
  queue.enqueue({ position: start, cost: 0, directions: [-1, -1, -1] });
  let counter = 0;
  while (queue.size() > 0) {
    ...
    const cacheKey = position.concat(directions.slice(directions.length - 3)).toString();
    if (minCost[cacheKey] <= cost) continue;
    minCost[cacheKey] = cost;

    for (let direction of range(4)) {

      // Cannot move 3 squares in the same direction
      if (directions.slice(directions.length - 3).every(d => d === direction)) continue;
      ...
      queue.enqueue({
        ...
        cost: newCost,
        directions: directions.slice(directions.length - 2).concat(direction)
      });
    }
  }
